...
from models.resenet_cifar import resnet18
import re
import glob



def get_all_task_checkpoints(ckpt_path):
    task_checkpoints = {}
    for filename in os.listdir(ckpt_path):
        if filename.endswith('.ckpt'):
            method_id = filename.split('-')[0]
            task_checkpoints[method_id] = os.path.join(ckpt_path, filename)
...
if __name__ == '__main__':
    ...
    for method_id, method_ckpt in task_checkpoints.items():
        args.ckpt_path = method_ckpt
        ...
        wandb_logger = WandbLogger(name=f"{method_id}_{args.run_name}", project=args.project, entity=args.entity,
